...
var modalImage_adjust = document.querySelector(
  ".modal-ADJUST .modal-body_ADJUST img"
);
...
class Pro {
  constructor(container, img, ma, tieude, name, price, icon) {
    ...
    this.element.classList.add("pro", "hide");
...
function detectInputChange() {
  ...
  const textVal = searchInput.value;
  if (textVal.length > 0) {
    // const searchInput = document.querySelector(".search-input-wrapper");
    if (searchInput) {
      searchInput.classList.add("maintain-hover");
    }
  } else {
    searchInput.classList.remove("maintain-hover");
  }
}
...
// Hien thong bao mua hang

var btnOpen = document.querySelector(".normal");
var thongbao = document.querySelector(".thongbao");
var iconClose = document.querySelector(".thongbao-body i");

function toggleModal() {
  thongbao.classList.toggle("hide");
}
